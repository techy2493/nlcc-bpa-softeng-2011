...
/**
 * A simple text Scanner2 which can parse primitive types and strings using
 * regular expressions.
 *
 * <p>A <code>Scanner2</code> breaks its input into tokens using a
 ...
 * <blockquote><pre>
 *      Scanner2 sc = new Scanner2(new File("myNumbers"));
 *      while (sc.hasNextLong()) {
 *          long aLong = sc.nextLong();
 *      }</pre></blockquote>
 *
 * <p>The Scanner2 can also use delimiters other than whitespace. This
 * example reads several items in from a string:
 *<blockquote><pre>
 *     String input = "1 fish 2 fish red fish blue fish";
 *     Scanner2 s = new Scanner2(input).useDelimiter("\\s*fish\\s*");
 ...
 * by a Scanner2 is as recognized by {@link java.lang.Character}.{@link
 * java.lang.Character#isWhitespace(char) isWhitespace}. The {@link #reset}
 * method will reset the value of the Scanner2's delimiter to the default
 ...
 *
 * <p>When a Scanner2 throws an {@link InputMismatchException}, the Scanner2
 ...
 * space at a time.
 *
 * <p> A Scanner2 can read text from any object which implements the {@link
 * java.lang.Readable} interface.  If an invocation of the underlying
 * readable's {@link java.lang.Readable#read} method throws an {@link
 * java.io.IOException} then the Scanner2 assumes that the end of the input
 * has been reached.  The most recent <tt>IOException</tt> thrown by the
 * underlying readable can be retrieved via the {@link #ioException} method.
 *
 * <p>When a <code>Scanner2</code> is closed, it will close its input source
 * if the source implements the {@link java.io.Closeable} interface.
 *
 * <p>A <code>Scanner2</code> is not safe for multithreaded use without
 * external synchronization.
 *
 * <p>Unless otherwise mentioned, passing a <code>null</code> parameter into
 * any method of a <code>Scanner2</code> will cause a
 * <code>NullPointerException</code> to be thrown.
 *
 * <p>A Scanner2 will default to interpreting numbers as decimal unless a
 * different radix has been set by using the {@link #useRadix} method. The
 * {@link #reset} method will reset the value of the Scanner2's radix to
 ...
 * formats as well as in the formats of the Scanner2's locale. A Scanner2's
 ...
 * Scanner2's locale to the initial locale regardless of whether it was
 * previously changed.
 *
 ...
 * @since   1.5
 */
public final class Scanner2 implements Iterator<String> {
    ...
    // The index into the buffer currently held by the Scanner2
    ...
    // A store of a position that the Scanner2 may fall back to
    private int savedScanner2Position = -1;
    ...
    // Boolean indicating if this Scanner2 has been closed
    private boolean closed = false;

    // The current radix used by this Scanner2
    private int radix = 10;

    // The default radix for this Scanner2
    private int defaultRadix = 10;

    // The locale used by this Scanner2
    ...
    private String buildIntegerPatternString() {
        ...
        // Scanner2 will do the right thing
    ...
    /**
     * Constructs a <code>Scanner2</code> that returns values scanned
     * from the specified source delimited by the specified pattern.
     *
     * @param  source A character source implementing the Readable interface
     * @param pattern A delimiting pattern
     * @return A Scanner2 with the specified source and pattern
     */
    private Scanner2(Readable source, Pattern pattern) {
        if (source == null)
    ...
    /**
     * Constructs a new <code>Scanner2</code> that produces values scanned
     * from the specified source.
     *
     * @param  source A character source implementing the {@link Readable}
     *         interface
     */
    public Scanner2(Readable source) {
     ...
     *         does not exist
     */
    public Scanner2(InputStream source, String charsetName) {
     ...
     *         not found
     */
    public Scanner2(File source, String charsetName)
        throws FileNotFoundException
    {
        this((ReadableByteChannel)(new FileInputStream(source).getChannel()),
             charsetName);
     ...
     */
    public Scanner2(ReadableByteChannel source) {
     ...
     *         does not exist
     */
    public Scanner2(ReadableByteChannel source, String charsetName) {
        this(makeReadable(source, charsetName), WHITESPACE_PATTERN);
    }
    ...
    private void saveState() {
        savedScanner2Position = position;
    }

    private void revertState() {
        this.position = savedScanner2Position;
        savedScanner2Position = -1;
        skipped = false;
    }
    ...
    private void useTypeCache() {
        if (closed)
            throw new IllegalStateException("Scanner2 closed");
        position = hasNextPosition;
        hasNextPattern = null;
        typeCache = null;
    }
    ...
    private boolean makeSpace() {
        clearCaches();
        int offset = savedScanner2Position == -1 ?
            position : savedScanner2Position;
        buf.position(offset);
    ...
    private void translateSavedIndexes(int offset) {
        if (savedScanner2Position != -1)
            savedScanner2Position -= offset;
    }
    ...
    // Throws if the Scanner2 is closed
    ...
    /**
     * Closes this Scanner2.
     *
     * <p> If this Scanner2 has not yet been closed then if its underlying
     * {@linkplain java.lang.Readable readable} also implements the {@link
     * java.io.Closeable} interface then the readable's <tt>close</tt> method
     * will be invoked.  If this Scanner2 is already closed then invoking this
     * method will have no effect.
     *
     * <p>Attempting to perform search operations after a Scanner2 has
     ...
     * <code>Scanner2</code>'s underlying <code>Readable</code>. This method
     * returns <code>null</code> if no such exception exists.
     *
     * @return the last exception thrown by this Scanner2's readable
     */
    ...
    /**
     * Returns the <code>Pattern</code> this <code>Scanner2</code> is currently
     * using to match delimiters.
     *
     * @return this Scanner2's delimiting pattern.
     */
    ...
    /**
     * Sets this Scanner2's delimiting pattern to the specified pattern.
     *
     * @param pattern A delimiting pattern
     * @return this Scanner2
     */
    public Scanner2 useDelimiter(Pattern pattern) {
        delimPattern = pattern;
        return this;
    }

    /**
     * Sets this Scanner2's delimiting pattern to a pattern constructed from
     ...
     *
     * <p> Invoking the {@link #reset} method will set the Scanner2's delimiter
     * to the <a href= "#default-delimiter">default</a>.
     *
     * @param pattern A string specifying a delimiting pattern
     * @return this Scanner2
     */
    public Scanner2 useDelimiter(String pattern) {
        delimPattern = patternCache.forName(pattern);
        return this;
    }

    /**
     * Returns this Scanner2's locale.
     *
     * <p>A Scanner2's locale affects many elements of its default
     * primitive matching regular expressions; see
     * <a href= "#localized-numbers">localized numbers</a> above.
     *
     * @return this Scanner2's locale
     */
    ...
    /**
     * Sets this Scanner2's locale to the specified locale.
     *
     * <p>A Scanner2's locale affects many elements of its default
     * primitive matching regular expressions; see
     * <a href= "#localized-numbers">localized numbers</a> above.
     *
     * <p>Invoking the {@link #reset} method will set the Scanner2's locale to
     * the <a href= "#initial-locale">initial locale</a>.
     *
     * @param locale A string specifying the locale to use
     * @return this Scanner2
     */
    public Scanner2 useLocale(Locale locale) {
    ...
    /**
     * Returns this Scanner2's default radix.
     *
     * <p>A Scanner2's radix affects elements of its default
     * number matching regular expressions; see
     * <a href= "#localized-numbers">localized numbers</a> above.
     *
     * @return the default radix of this Scanner2
     */
    public int radix() {
    ...
    /**
     * Sets this Scanner2's default radix to the specified radix.
     *
     * <p>A Scanner2's radix affects elements of its default
     * number matching regular expressions; see
     * <a href= "#localized-numbers">localized numbers</a> above.
     ...
     *
     * <p>Invoking the {@link #reset} method will set the Scanner2's radix to
     * <code>10</code>.
     *
     ...
     */
    public Scanner2 useRadix(int radix) {
     ...
     * by this Scanner2. This method throws <code>IllegalStateException</code>
     * if no match has been performed, or if the last match was
     * not successful.
     *
     * <p>The various <code>next</code>methods of <code>Scanner2</code>
    ...
    /**
     * <p>Returns the string representation of this <code>Scanner2</code>. The
     * string representation of a <code>Scanner2</code> contains information
     * that may be useful for debugging. The exact format is unspecified.
     *
     * @return  The string representation of this Scanner2
     */
    public String toString() {
        StringBuilder sb = new StringBuilder();
	sb.append("java.util.Scanner2");
    ...
    /**
     * Returns true if this Scanner2 has another token in its input.
     * This method may block while waiting for input to scan.
     * The Scanner2 does not advance past any input.
     *
     * @return true if and only if this Scanner2 has another token
     * @throws IllegalStateException if this Scanner2 is closed
    ...
    /**
     * Finds and returns the next complete token from this Scanner2.
     ...
     * specified string. The Scanner2 does not advance past any input.
     *
     ...
     * @return true if and only if this Scanner2 has another token matching
     ...
     * specified string.  If the match is successful, the Scanner2 advances
     ...
     * If the match is successful, the Scanner2 advances past the input that
     * matched the pattern.
     *
     * @param pattern the pattern to scan for
     * @return the next token
    ...
    /**
     * Returns true if there is another line in the input of this Scanner2.
     * This method may block while waiting for input. The Scanner2 does not
     * advance past any input.
     *
     * @return true if and only if this Scanner2 has another line of input
     * @throws IllegalStateException if this Scanner2 is closed
     */
    ...
    /**
     * Advances this Scanner2 past the current line and returns the input
     * that was skipped.
     *
    ...
    //Attempts to take the next line of input. This does not block.
    public String nextLineNoBlock() {
        if (hasNextPattern == linePattern())
            return getCachedResult();
        clearCaches();

        String result = findWithinHorizonNoBlock(linePattern, 0);
        if (result == null)
            throw new NoSuchElementException("No line found");
        MatchResult mr = this.match();
        String lineSep = mr.group(1);
     ...
     * Scanner2 advances past the input that matched and returns the string that
     * matched the pattern.
     * If no such pattern is detected in the input up to the next line
     * separator, then <code>null</code> is returned and the Scanner2's
     ...
     * Scanner2 advances past the input that matched and returns the string
     * that matched the pattern. If no such pattern is detected then the
     * null is returned and the Scanner2's position remains unchanged. This
     * method may block waiting for input that matches the pattern.
     *
     * <p>A Scanner2 will never search more than <code>horizon</code> code
     * points beyond its current position. Note that a match may be clipped
     * by the horizon; that is, an arbitrary match result may have been
     * different if the horizon had been larger. The Scanner2 treats the
    ...
    public String findWithinHorizon(Pattern pattern, int horizon) {
        ...
        public String findWithinHorizonNoBlock(Pattern pattern, int horizon) {
        ensureOpen();
        if (pattern == null)
            throw new NullPointerException();
        if (horizon < 0)
     ...
     * the Scanner2's current position, patterns that can match a lot of
     * input (".*", for example) may cause the Scanner2 to buffer a large
     * amount of input.
     *
     ...
     * @throws IllegalStateException if this Scanner2 is closed
     */
    public Scanner2 skip(Pattern pattern) {
        ensureOpen();
        if (pattern == null)
            throw new NullPointerException();
        clearCaches();
     ...
     * @throws IllegalStateException if this Scanner2 is closed
     */
    public Scanner2 skip(String pattern) {
    ...
    /**
     * Returns true if the next token in this Scanner2's input can be
     * interpreted as a boolean value using a case insensitive pattern
     * created from the string "true|false".  The Scanner2 does not
     * advance past the input that matched.
     *
     * @return true if and only if this Scanner2's next token is a valid
     *         boolean value
     * @throws IllegalStateException if this Scanner2 is closed
     */
     ...
     * {@link #nextByte} method. The Scanner2 does not advance past any input.
     *
     * @return true if and only if this Scanner2's next token is a valid
     *         byte value
     * @throws IllegalStateException if this Scanner2 is closed
     ...
     * is the default radix of this Scanner2.
     *
     ...
     * described below. If the translation is successful, the Scanner2 advances
     ...
     * {@link #nextShort} method. The Scanner2 does not advance past any input.
     *
     * @return true if and only if this Scanner2's next token is a valid
     ...
     * {@link #nextInt} method. The Scanner2 does not advance past any input.
     *
     * @return true if and only if this Scanner2's next token is a valid
     *         int value
     * @throws IllegalStateException if this Scanner2 is closed
     ...
     * {@link #nextLong} method. The Scanner2 does not advance past any input.
     *
     * @return true if and only if this Scanner2's next token is a valid
     *         long value
     * @throws IllegalStateException if this Scanner2 is closed
     ...
     * method. The Scanner2 does not advance past any input.
     *
     * @return true if and only if this Scanner2's next token is a valid
     *         float value
     * @throws IllegalStateException if this Scanner2 is closed
     ...
     * If the translation is successful, the Scanner2 advances past the input
     * that matched.
     *
     * <p> If the next token matches the <a
     * href="#Float-regex"><i>Float</i></a> regular expression defined above
     ...
     * {@link #nextBigInteger} method. The Scanner2 does not advance past any
     * input.
     *
     * @return true if and only if this Scanner2's next token is a valid
     ...
     * the {@link #nextBigInteger} method. The Scanner2 does not advance past
     * any input.
     *
     ...
     * {@link #nextBigDecimal} method. The Scanner2 does not advance past any
     * input.
     *
     * @return true if and only if this Scanner2's next token is a valid
    ...
    /**
     * Resets this Scanner2.
     *
     * <p> Resetting a Scanner2 discards all of its explicit state
     * information which may have been changed by invocations of {@link
     * #useDelimiter}, {@link #useLocale}, or {@link #useRadix}.
     *
     * <p> An invocation of this method of the form
     * <tt>Scanner2.reset()</tt> behaves in exactly the same way as the
     * invocation
     *
     * <blockquote><pre>
     *   Scanner2.useDelimiter("\\p{javaWhitespace}+")
     *          .useLocale(Locale.getDefault())
     *          .useRadix(10);
     * </pre></blockquote>
     *
